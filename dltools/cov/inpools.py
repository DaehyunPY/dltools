...
def cov1x11_inpools(
    ...
    def analyzer(
        ...
        def f(row: pyspark.sql.Row) -> typing.Iterator[tuple]:
            ...
            for at0, d1 in product(x0, x1):
                yield (at0 + 1, d1["at"] + 1, 0)

            for at0, d2 in product(x0, x2):
                yield (at0 + 1, 0, d2["at"] + 1)
            ...
            for at0, d1, d2 in product(x0, x1, x2):
                if len({d1["arg"], d2["arg"]}) != 2:
                    continue
                yield (at0 + 1, d1["at"] + 1, d2["at"] + 1)
